...
| VHDL : `bound check failure` à l'exécution sur une multiplication par un littéral (`resize(x, 64) * 2`) | GHDL refuse `vecteur_large * littéral_entier` même pour de petites valeurs | `shift_left(x, 1)` — le message ne nomme pas l'opérateur, isoler l'expression dans un repro de 10 lignes |
| VHDL : `bound check failure` sur `unsigned + delta` avec `delta` de type `integer` | l'opérateur numeric_std `unsigned + INTEGER` attend en réalité un **NATURAL** : une valeur négative viole le sous-type | écrire les cas en branches explicites (`+1`, `-1`, inchangé) plutôt que d'ajouter un delta signé |
| VHDL : `bound check failure` dans une boucle de virgule fixe | le format Q choisi ne couvre pas le **pire cas intermédiaire** (Q28 sur 32 bits porte ±8,0 et `2·zr·zi` atteint exactement 8,0) | dimensionner le format sur l'intermédiaire le plus grand, pas sur les signaux finaux (Q26 → ±32) ; l'annoncer sur un port `o_cfg_frac` |
| cocotb runner + GHDL : `cannot find entity or configuration <top>` | le runner exécute la simulation dans `test_dir` alors que l'élaboration va dans `build_dir` | `test_args=["--std=08", "--workdir=<build_dir>", "-P<build_dir>"]` (ou `test_dir = build_dir`) |
| cocotb : banc de test à 0,6 ms de Python par cycle d'horloge | attente par front à chaque cycle (5 000 cycles/s seulement) | attendre la fin sur le signal de fin : `await dut.o_done.value_change` (×1000 sur la durée du banc) |
